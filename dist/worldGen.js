    ...
    for (let x = 0; x < size; x++) {
        for (let y = 0; y < size; y++) {
            ...
            if (currentNoiseMode == "orig") {
                ...
                if (y == 0) {
                    let oldns = ns;
                    let temp1 = simplex.noise4D(test.x / 8, (1 / 2 - 16) / 8, test.z / 8, gen.z) /
                        2 +
                        0.5;
                    let temp2 = simplex.noise4D(test.x / 8, (size / 2 - 16) / 8, test.z / 8, gen.z) /
                        2 +
                        0.5;
                    ...
                    let oldns = ns;
                    let temp1 = simplex.noise4D(test.x / 8, (0 / 2 - 16) / 8, test.z / 8, gen.z) /
                        2 +
                        0.5;
                    let temp2 = simplex.noise4D(test.x / 8, ((size - 1) / 2 - 16) / 8, test.z / 8, gen.z) /
                        2 +
                        0.5;
                    ns = (oldns + temp1 + temp2) / 3;
                }
            ...
            let rng = Math.random();
            let pieces2 = Object.keys(gen.special).sort((a, b) => parseFloat(b) - parseFloat(a));
